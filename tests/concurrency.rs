...
fn concurrent_mixed_ops_string_keys() {
    let m: Arc<FlatMap<String, usize>> = Arc::new(FlatMap::with_capacity(1024));
    let n_threads = 6;
    let iters = 3_000;
...
fn concurrent_integer_keys_contention() {
    let m: Arc<FlatMap<u64, u64>> = Arc::new(FlatMap::with_capacity(2048));
    let n_threads = 6;
    let iters = 1_500;
    let barrier = Arc::new(Barrier::new(n_threads));
    ...
    for t in 0..n_threads {
        ...
        handles.push(thread::spawn(move || {
            ...
            for i in 0..iters {
                ...
                match (i + t) % 3 {
                    0 => { map.insert(k, i as u64); }
                    1 => { let _ = map.get(&k); }
                    _ => { let _ = map.remove(k); }
                }
            }
        }));
    }
    ...
    for k in &hot_keys { let _ = m.get(k); }
}
...
fn concurrent_load_or_insert_once_under_race() {
    ...
    let m: Arc<FlatMap<i32, i32>> = Arc::new(FlatMap::new());
    let called = Arc::new(AtomicI32::new(0));

    let workers = std::cmp::max(2, std::thread::available_parallelism().map(|n| n.get()).unwrap_or(2));
    let mut handles = Vec::new();
    for _ in 0..workers {
        let map = m.clone();
        let c = called.clone();
...
fn double_buffer_consistency_under_updates() {
    ...
    for i in 1..=NUM_KEYS { let _ = m.insert(i, i); }
...
fn seqlock_consistency_stress_aba() {
    ...
    let m: Arc<FlatMap<i32, Pair>> = Arc::new(FlatMap::new());
    ...
    for _ in 0..writer_n {
        ...
        handles.push(thread::spawn(move || {
            ...
            while !s.load(std::sync::atomic::Ordering::Relaxed) {
                ...
                let val = Pair { x: seq as u64, y: !(seq as u64) };
...
fn load_delete_race_semantics() {
    ...
    let m: Arc<FlatMap<String, u32>> = Arc::new(FlatMap::new());
    let key = "k".to_string();
    ...
    // readers
    let reader_n = std::cmp::max(2, std::thread::available_parallelism().map(|n| n.get()).unwrap_or(2));
    for _ in 0..reader_n {
        let map = m.clone();
        let s = stop.clone();
        ...
        handles.push(thread::spawn(move || {
            while !s.load(std::sync::atomic::Ordering::Relaxed) {
                if let Some(v) = map.get(&key_r) {
                    if v == 0 { an.fetch_add(1, std::sync::atomic::Ordering::Relaxed); }
                }
            }
        }));
    }
    ...
    for h in handles { h.join().unwrap(); }
    assert_eq!(anomalies.load(std::sync::atomic::Ordering::Relaxed), 0, "observed ok==true && value==0 under load/delete race");
}

#[test]
fn key_torn_read_stress() {
    #[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
    struct BigKey { a: u64, b: u64 }
    use std::sync::Arc;
    use std::thread;
    use std::time::Duration;
    ...
    let m: Arc<FlatMap<BigKey, i32>> = Arc::new(FlatMap::new());
    ...
    for r in 0..4 {
        ...
        handles.push(thread::spawn(move || {
            while !s.load(std::sync::atomic::Ordering::Relaxed) {
                for i in r..N { let _ = map.get(&keys_c[i]); }
            }
        }));
    }
    ...
    for w in 0..2 {
        ...
        handles.push(thread::spawn(move || {
            while !s.load(std::sync::atomic::Ordering::Relaxed) {
                for i in w..N { let k = keys_c[i]; let _ = map.remove(k); let _ = map.insert(k, i as i32); }
                std::thread::yield_now();
            }
        }));
    }
